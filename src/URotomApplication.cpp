...
static URotomContext* ResizeContext = nullptr;
...
URotomApplication::URotomApplication() {
	mWindow = nullptr;
	mContext = nullptr;
}

bool URotomApplication::Setup() {
	// Initialize GLFW
	if (!glfwInit())
		return false;
	...
	mWindow = glfwCreateWindow(1280, 720, "Rotom", nullptr, nullptr);
	if (mWindow == nullptr) {
		glfwTerminate();
		return false;
	}
	...
	mContext = new URotomContext();
	ResizeContext = mContext;

	return true;
}

bool URotomApplication::Teardown() {
...
bool URotomApplication::Execute(float deltaTime) {
