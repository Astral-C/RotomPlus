...
void CPointSpriteManager::SetBillboardTextureData(uint8_t* data, int TextureIndex){
    if(data == nullptr) return;

    glBindTexture(GL_TEXTURE_2D_ARRAY, mTextureID);
    glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, TextureIndex, mBillboardResolution, mBillboardResolution, 1, GL_RGBA, GL_UNSIGNED_BYTE, data);
    glBindTexture(GL_TEXTURE_2D_ARRAY, 0);
}

void CPointSpriteManager::UpdateData(std::vector<CPointSprite>& sprites){
    mBillboards.clear();
